...
module.exports = class DatabaseConnectionUrlBuilder {
  ...
  async build() {
    utils.copyServerlessEnvironmentValuesToProcessEnvironment(this.serverless);

    // let connectionUrl = process.env.DB_CONNECTION_URL;
    const rdsSecretStore = process.env.RDS_SECRET_STORE;
    const secretsManager = new AWS.SecretsManager();
    const getSecretValueRequest = {
      SecretId: rdsSecretStore
    };
  ...
  buildDatabaseConnectionUrlFromIndividualProperties() {
    const missingProperty = this.checkForMissingDatabaseConnectionIndividualProperties();

    if (missingProperty) {
      this.serverless.cli.log(
        `Missing ${missingProperty} in the environment variables`
      );
      process.exit(1);
    }
  ...
  getDbPasswordIndividualProperty() {
    const dbPassword = this.options.dbPassword || process.env.DB_PASSWORD;

    if (!_.isNil(dbPassword)) return dbPassword;
  ...
  dbPasswordIndividualPropertyIsSet() {
    return (
      Object.prototype.hasOwnProperty.call(this.options, "dbPassword") ||
